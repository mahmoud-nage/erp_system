...
@push('scripts')
    {{$dataTable->scripts()}}
          ...
          var s_admin = {!! json_encode(auth()->guard('admin')->user()->hasRole('super admin')) !!};
                ...
                if(!s_admin){
                    var create = {!! json_encode(auth()->guard('admin')->user()->can('create permission')) !!};
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4">
                <div class="row ">
                    <div class="col-sm-12">
                        <table class="table table-bordered table-striped" style="@if(app()->getLocale() == 'ar') direction:rtl @endif" role="grid" aria-describedby="example1_info">
                                ...
                                @foreach($records as $record)
                                        <tr>
                                            ...
                                            <td class="text-center" >
                                                <a class="btn btn-info btn-group-vertical" href="{{route('permission.edit', $record->id)}}">
                                                    <i class="fas fa-user-edit"></i>
                                                </a>
                                
                                                <form action="{{route('permission.destroy', $record->id)}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        <div class="row"><div class="col-sm-12 col-md-5"><div class="dataTables_info" id="example1_info" role="status" aria-live="polite">Showing 1 to 10 of {{floor($records->count()/10)}} entries</div></div><div class="col-sm-12 col-md-7"><div class="float-md-right"> </div></div></div></div>
