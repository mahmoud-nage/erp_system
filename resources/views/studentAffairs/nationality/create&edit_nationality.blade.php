...
{{__('lang.new_nationality')}}
@else
{{__('lang.edit_nationality')}}
      ...
      <div class="card">
        <div class="card-content">
          <div class="card-body">
            ...
            @if(!empty($record) && $record['name_'.app()->getLocale()] == null)
                   {{-- @dd('create') --}}
                   <div class=" content-header col-lg-12">{{__('lang.new_nationality')}}</div><br>
                    <form class="form-horizontal" method="POST" action="{{route('nationality.store')}}">

                @else
                   {{-- @dd('update') --}}
                   <div class=" content-header col-lg-12">{{__('lang.edit_nationality')}}</div><br>
                    <form class="form-horizontal" method="POST" action="{{route('nationality.update', $record->id)}}">
